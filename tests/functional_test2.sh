...
ssh -o StrictHostKeyChecking=no -i $PATH_TO_PRIVATE_SSH_KEY $VM_ADMIN_USERNAME@$PUBLIC_IP
...
# az vm create -g $RG -n $vm --image $base_image 
# --admin-username $admin_username --ssh-key-value @${ssh_key_path} 
# --boot-diagnostics-storage $storage --size Standard_D2ds_v5 
# --accelerated-network true --nic-delete-option Delete --os-disk-delete-option 
# Delete --custom-data $temp_dir/customdata.yml