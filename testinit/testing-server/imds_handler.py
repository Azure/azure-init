...
class IMDSHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.headers.get("Metadata") != "true":
            ...
            error_response = {
                "error": "Bad Request",
                "message": "Metadata header not found",
            }
        ...
        if parsed_url.path == "/metadata/instance":
            ...
            error_response = {
                "error": "Not Found",
                "message": f"Endpoint {parsed_url.path} not found",
            }
            self.wfile.write(json.dumps(error_response).encode())
...
MOCK_INSTANCE_METADATA_EXTENDED = {
    "compute": {
        ...
        "storageProfile": {
            ...
            "osDisk": {
                ...
                "diskSizeGB": "30",
                "encryptionSettings": {
                    "diskEncryptionKey": {"secretUrl": "", "sourceVault": {"id": ""}},
                    "enabled": "false",
                    "keyEncryptionKey": {"keyUrl": "", "sourceVault": {"id": ""}},
                },
                "image": {"uri": ""},
                "managedDisk": {
        ...
        "tagsList": [
            {"name": "deployment_end", "value": "2025-10-08T13:52:29.401086Z"},
            {"name": "deployment_start", "value": "2025-10-08T13:51:50.291058Z"},
        ],
        "userData": "",
    ...
    "extended": {"compute": {"hasCustomData": False, "ppsType": "None"}},
}
