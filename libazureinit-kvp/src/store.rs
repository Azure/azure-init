...
//! KVP pool storage and file locking.
...
/// Unique-key limit for insert and load operations.
const MAX_UNIQUE_KEYS: usize = 1024;

const RECORD_SIZE: usize = WIRE_MAX_KEY_BYTES + WIRE_MAX_VALUE_BYTES;

/// Selects the Hyper-V pool file to access.
///
/// Use [`Guest`](Self::Guest) for guest-produced diagnostics and reports.
/// Other pools are generally populated by the host or integration daemon.
...
pub enum KvpPool {
    ...
    Guest = 1,
    /// Guest details generated by the daemon, not read from `.kvp_pool_2`.
    Auto = 2,
    /// Host-originated data describing the host (`.kvp_pool_3`).
    AutoExternal = 3,
    /// Internal pool (`.kvp_pool_4`); not intended for application telemetry.
    AutoInternal = 4,
}
...
/// Size policy for keys and values written through [`KvpPoolStore`].
///
/// Limits count UTF-8 bytes, not characters or UTF-16 code units. This policy
/// does not restrict reading records written with a different mode.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum PoolMode {
    /// Host-compatible limits: keys up to 254 bytes and values up to 1,022 bytes.
    /// Recommended for data exchanged with the host.
    Safe,
    /// Full pool-field widths: keys up to 512 bytes and values up to 2,048 bytes.
    /// Such records may be truncated during transport to the host.
    Unsafe,
}
...
/// Reads and writes UTF-8 key/value records in one Linux Hyper-V pool file.
///
/// Constructors select a path and size policy without performing I/O. Each
/// operation opens and locks the file as needed. The parent directory must
/// exist before writing.
///
/// [`insert`](Self::insert) updates a key, while [`append`](Self::append)
/// preserves duplicates. Use [`dump`](Self::dump) for every physical record
/// in pool order, or [`entries`](Self::entries) for the last value of each key.
/// [`load`](Self::load) replaces the entire pool.
///
/// Records persist across boots until explicitly cleared. Writes reject empty
/// keys and NUL bytes. An I/O failure during a write may leave partial changes.
///
/// See the [KVP contract] for record layout and transport details.
///
...
impl KvpPoolStore {
    /// Appends a key/value pair, preserving existing records and duplicates.
    ///
    /// Record count is not capped. Use [`insert`](Self::insert) to replace a
    /// key's existing values.
    ...
    /// Appends records in iteration order under one exclusive lock.
    ///
    /// Existing records and duplicates are kept, and record count is not
    /// capped. Empty input is a no-op. Inputs are validated before writing.
    /// Cooperating writers cannot interleave with the batch; an I/O error may
    /// leave only part of it written.
    ...
    /// Removes all records. A missing pool file is left absent.
    ...
    /// Clears the pool if its modification time is at or before system boot.
    ///
    /// The check and removal hold an exclusive lock. A missing file is a no-op.
    ...
    /// Removes all records matching `key`, returning whether any were found.
    ///
    ...
    /// Removal may change the order of remaining records.
    ...
    /// Removes all records whose key appears in `keys` under one exclusive lock.
    ///
    /// Returns the number of records removed, counting duplicates separately.
    /// Like [`delete`](Self::delete), key size is not capped. Empty input is a
    /// no-op; a missing file returns `Ok(0)`.
    ///
    /// Remaining records may be reordered.
    ...
    /// Returns every key/value record in pool order, including duplicates.
    ///
    /// A missing file returns an empty list. Invalid record layout or UTF-8,
    /// or a file-access error, fails the call without returning partial results.
    ...
    /// Returns one value per key, using the last record when keys repeat.
    ///
    /// A missing file returns an empty map. Use [`dump`](Self::dump) to retain
    /// duplicate records and their order.
    ...
    /// Inserts a key/value pair or replaces an existing key's value.
    ///
    /// Existing duplicates of the key are collapsed into one record. Adding
    /// a new key fails if the pool already contains 1,024 distinct keys.
    ...
    /// Returns whether the pool's modification time is at or before system boot.
    ...
    /// Returns the number of physical records, including duplicate keys.
    ...
    /// Selects a pool in the default `/var/lib/hyperv` directory without I/O.
    pub fn new(pool: KvpPool, mode: PoolMode) -> Result<Self, KvpError> {
        Self::new_in(pool, KvpPool::default_dir(), mode)
    }

    /// Selects a pool in `dir` without opening or creating it.
    ///
    /// The file name is derived from [`KvpPool::file_name`]. The directory must
    /// exist before performing a write that creates the file.
    pub fn new_in(
        pool: KvpPool,
        dir: impl AsRef<Path>,
        mode: PoolMode,
    ...
    /// Returns the configured pool-file path.
    ...
    /// Replaces the entire pool in iteration order under one exclusive lock.
    ///
    /// Duplicates are preserved, with at most 1,024 distinct keys. Empty input
    /// clears the pool. Use [`append_multiple`](Self::append_multiple) to keep
    /// existing records instead.
    ///
    /// Invalid input leaves the pool unchanged; a storage error after truncation
    /// may leave a partial replacement. A successful call updates modification
    /// time. An existing file with invalid record framing must be explicitly
    /// cleared with [`clear`](Self::clear) before it can be replaced.
    ...
    /// Returns the last value for `key`, or `None` when absent.
    ///
    /// Rejects empty keys and keys containing null bytes; key size is
